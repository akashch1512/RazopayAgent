...
def test_context_builder_handles_every_configured_webhook_event(event_type: str, path: pathlib.Path) -> None:
    ...
    assert len(messages) == 2, "expected exactly [system prompt, case brief]"

    system_message, case_brief_message = messages
    assert system_message.content and business.name in system_message.content
    assert case_brief_message.content and event_type in case_brief_message.content

    assert state["case_id"] == case.id
    assert state["business_id"] == case.business_id
    assert state["razorpay_account_id"] == case.razorpay_account_id
    assert state["customer_timezone"]  # always resolves to something, at worst the default
