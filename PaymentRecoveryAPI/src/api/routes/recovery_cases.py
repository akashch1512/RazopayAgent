...
async def submit_customer_feedback(
    ...
    (`src.services.recovery.ingestion`) - a reply is just another event.
    """
    try:
        case = await case_repo.read_case_by_id(case_id=case_id)
    except EntityDoesNotExist as exc:
...
@router.post(
    path="/{case_id}/mark-paid",
    name="recovery-cases:mark-paid",
    status_code=fastapi.status.HTTP_200_OK,
)
async def mark_recovery_case_paid(
    ...
    """
    A human confirms this case is paid (the demo dashboard's "mark as paid",
    or ops closing it out of band). Synthesized as a `payment.captured` event
    so it flows through the same pipeline as a real resolving webhook and the
    case is force-closed to RESOLVED.
    """
    try:
        case = await case_repo.read_case_by_id(case_id=case_id)
    except EntityDoesNotExist as exc:
    ...
    case, _event = await record_manual_resolution(
        case_repo=case_repo, webhook_repo=webhook_repo, case=case
    )
