...
def _configure_celery_logging(**_kwargs: object) -> None:
    """Stop Celery from installing its own handlers - route task logs through
    the same stdout JSON/text pipeline the API uses."""
    configure_logging()
...
celery_app = Celery(
    ...
    include=[
        ...
        "src.workers.tasks.scheduled_actions",
    ],
)
...
celery_app.conf.update(
    ...
    beat_schedule={
        ...
        "dropoff-poll-businesses": {
          ...
          "scheduled-actions-reconcile": {
            "task": names.SCHEDULED_ACTION_RECONCILE_TASK,
            "schedule": float(settings.SCHEDULED_ACTION_SWEEP_INTERVAL_SECONDS),
            "options": {"queue": settings.WEBHOOK_QUEUE_NAME, "priority": 1},
          },
    },
)
