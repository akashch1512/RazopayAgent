...
pipeline as a real webhook (`src.services.recovery.ingestion`), tagged
...
async def _poll_one_business(
    ...
    *,
    ...
    if headers is None:
        logger.warning(f"business id={business.id}: no Razorpay auth available, skipping drop-off poll.")
        return 0
    if is_demo:
        logger.warning(
    ...
    try:
        ...
        logger.error(f"business id={business.id}: drop-off poll failed: {exc!r}")
        return 0
...
async def _poll_due_businesses() -> dict[str, int]:
    ...
    if polled:
        logger.info(f"drop-off poll: businesses_polled={polled} dropoffs_found={dropoffs_found}")
